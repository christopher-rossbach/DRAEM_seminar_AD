...
To be able to run more experiments and use also the lower end GPUs of the HPC, I implemented micro-batch training and used automatic mixed precision (AMP) for the forward pass, while using full precision for loss calculation and evaluation to ensure numerical stability.
In addition I fixed a bug where the last incomplete batch of each epoch was weighted incorrectly during training, which was causing instability in the training.
...
In fact, my reproduced AUROC metrics (image- and pixel-level) on the combined MVTec dataset are within 0.1 percentage points of the values reported in the paper, while the pixel AP value improved from 68.4\% to 69.7\%, probably due to more stable training.
...
For all blending methods, I kept the anomaly mask generation the same and just varied the strength depending on the blending method.
...
However, on some object categories, the other methods outperform the Poisson blending.
The best general performance was achieved by combining the Poisson and the Perlin-$\beta$ blending methods, by randomly choosing one for each generated anomaly.
