...
Besides a reduced memory footprint, the implementation of micro-batches fixed a bug in the original code, where the last incomplete batch of each epoch was weighted incorrectly during training (see Figure~\ref{fig:smooth-loss-curve}).
...
In fact, my reproduced AUROC metrics (image and pixel level) on the combined MVTec dataset are within a 0.1\% interval of the values reported in the paper, while the pixel AP value improved from 68.4 to 69.7, probably as a result of stabilized training (average of 4 independent runs)\footnote{The metrics on a per-category basis differ notably more, but this can be attributed to the natural variance of the training process. For concrete values see Figure~\ref{fig:blending_comparison_heatmap}.}.
...
My main contribution, implementing additional blending methods for anomaly generation, is part of the data loading pipeline and can therefore be found in the \texttt{data\_loader.py} script.
...
As can be seen, the Poisson image interpolation yields, on average, the best results\footnote{This method was suggested by my supervisor Johanna Müller.}.
...
The combination of methods was done by randomly selecting one blending method for each training image.
