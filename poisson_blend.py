# copied from https://github.com/jemtan/PII/blob/main/poissonBlend.py
from os import path
import numpy as np
import scipy.sparse
...
def make_boolean_mask(mask):
    # mask may be (H,W,1) or (H,W) or (s,H,W,1)
    m = np.squeeze(mask)
    return (m > 0)  # boolean

def estimate_mix_map(result, img, anomaly):
    denom = anomaly - img
    w = np.zeros_like(denom, dtype=np.float64)

    # valid where denom not near zero
    valid = np.abs(denom) > 1e-6
    w[valid] = (result[valid] - img[valid]) / denom[valid]

    # clip to [0,1]
    w = np.clip(w, 0.0, 1.0)

    # average along the channel axis:
